...
class SpamWindow {
    ...
    bool canSend() {
        ...
        for (int i=int(sends.Length)-1;i>=0;i--) if (now - sends[i] > float(kSpamWindowMs)) sends.RemoveAt(i);
...
void SendChat(const string &in text) {
    ...
    print("\t"+toSend);
    auto pg=GetApp().CurrentPlayground; if(pg is null) { warn("No playground"); return; }
    if (toSend.Length>2000) pg.Interface.ChatEntry = toSend.SubStr(0,2000)+" (...)"; else pg.Interface.ChatEntry = toSend;
}
...
// Flow
void HandleSelector(int selIdx) {
    if (selIdx<=0 || !Setting_Power || g_blockInput) return;
    if (!g_state.waiting) {
        if(selIdx>=1 && selIdx<=4) { g_state.waiting=true; g_state.group=selIdx; g_state.openedAt=float(Time::Now); }
        return;
    } else {
        if(!g_state.debounceOk()) return;
        int slot = SlotForSelector(selIdx);
        ...
        if (msg.Length>0) {
            ...
            SendChat(clipped);
            AddToastForSelector(clipped, selIdx);
