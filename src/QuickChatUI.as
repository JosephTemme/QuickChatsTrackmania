...
void AddLog(const string &in txt, const vec4 &in col) {
    LogItem@ it = LogItem();
    it.text = txt;
    it.t0 = float(Time::Now);
    it.col = col;
    g_log.InsertLast(it);
}

void RenderLog() {
    if (!Setting_LogSentShow && !Setting_LogSpamShow) {
        return;
    }
    int baseX = Setting_SentLogOffsetX;
    int baseY = Setting_SentLogOffsetY;
    int rowH = int(Setting_SentLogRowPx * Setting_SentLogScale);
    for (int i = int(g_log.Length) - 1; i >= 0; i--) {
        auto t = g_log[i];
        float age = float(Time::Now) - t.t0;
        if (age > float(Setting_OverlayDurationMs)) {
            g_log.RemoveAt(i);
            continue;
        }
...
void DrawOneBlob(const string &in text, int x, int y) {
    ...
    int flags = UI::WindowFlags::NoTitleBar | UI::WindowFlags::AlwaysAutoResize | UI::WindowFlags::NoSavedSettings | UI::WindowFlags::NoMove;
    if (UI::Begin("##qc_blob_" + x + "_" + y, flags)) {
        UI::Text(text);
    }
    UI::End();
    UI::PopStyleVar(2);
...
void DrawHintRect(int group) {
    ...
    int linePad = int(float(Setting_RectLineSpacing) * Setting_RectScale);
    UI::PushStyleColor(UI::Col::WindowBg, col);
    ...
    if (UI::Begin("##qc_rect", flags)) {
        if (Setting_RectShowHeader) {
            UI::Text("Selector " + group);
        }
...
void GetBlobPosForSelector(int selectorIndex, int &out x, int &out y) {
...
void AddToastForSelector(const string &in msg, int selectorIndex) {
    if (Setting_UseRectHint) {
        return;
    }
    int bx, by;
    GetBlobPosForSelector(selectorIndex, bx, by);
