...
string KeyName(VirtualKey k) {
    if (k >= VirtualKey::A && k <= VirtualKey::Z) {
        int offset = int(k) - int(VirtualKey::A);
        return "" + string("ABCDEFGHIJKLMNOPQRSTUVWXYZ").SubStr(offset,1);
    }
...
int SlotForSelector(int selIdx) {
    if (selIdx==2) return 0;
    if (selIdx==1) return 1;
    if (selIdx==3) return 2;
    if (selIdx==4) return 3;
    return 0;
}
