// Keyboard
[Setting name="Quick chat 1 Message" category="Quick Chats"]
string Setting_QuickChat1Message = "This is Rocket League!";

[Setting name="Quick chat 2 Message" category="Quick Chats"]
string Setting_QuickChat2Message = "OMG!";

[Setting name="Quick chat 3 Message" category="Quick Chats"]
string Setting_QuickChat3Message = "Nooooo!";

[Setting name="Quick chat 4 Message" category="Quick Chats"]
string Setting_QuickChat4Message = "Okay.";

[Setting name="Quick chat 1 Binding" category="Quick Chats"]
VirtualKey Setting_QuickChat1Binding = VirtualKey::Up;

[Setting name="Quick chat 2 Binding" category="Quick Chats"]
VirtualKey Setting_QuickChat2Binding = VirtualKey::Right;

[Setting name="Quick chat 3 Binding" category="Quick Chats"]
VirtualKey Setting_QuickChat3Binding = VirtualKey::Down;

[Setting name="Quick chat 4 Binding" category="Quick Chats"]
VirtualKey Setting_QuickChat4Binding = VirtualKey::Left;

// Controller (Xbox)
[Setting name="Quick chat 5 Message" category="Quick Chats"]
string Setting_QuickChat5Message = "Need boost!";

[Setting name="Quick chat 6 Message" category="Quick Chats"]
string Setting_QuickChat6Message = "Go for it!";

[Setting name="Quick chat 7 Message" category="Quick Chats"]
string Setting_QuickChat7Message = "$#@%!";

[Setting name="Quick chat 8 Message" category="Quick Chats"]
string Setting_QuickChat8Message = "gg";

[Setting name="Quick chat 5 Binding" category="Quick Chats"]
Button Setting_QuickChat5Binding = Button::Up;

[Setting name="Quick chat 6 Binding" category="Quick Chats"]
Button Setting_QuickChat6Binding = Button::Right;

[Setting name="Quick chat 7 Binding" category="Quick Chats"]
Button Setting_QuickChat7Binding = Button::Down;

[Setting name="Quick chat 8 Binding" category="Quick Chats"]
Button Setting_QuickChat8Binding = Button::Left;